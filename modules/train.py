...
def train(log_dir, args):
    ...
    checkpoint = tf.train.Checkpoint(step=tf.Variable(1), optimizer=opt, las=speech_model.model)
    ...
    train_step_signature = [tf.TensorSpec(shape=(None, None, args.num_mels), dtype=tf.float32),
    ...
    def train_step_non_teacher(inp, tar):
        ...
        with tf.GradientTape() as tape:
            for t in range(1, tf.shape(tar)[1]):
                ...
                tar_weight = tf.cast(tf.logical_not(tf.math.equal(tar[:, t], 0)), tf.int32)
                train_acc(tar[:, t], predictions[:, -1, :], sample_weight=tar_weight)
        ...
        if (1 + epoch) % args.eval_interval == 0:
            ...
            for (batch, (input, label)) in enumerate(valid_dataset):
                ...
                if batch % (valid_steps // 10) == 0:
                    ...
                    plot_alignment(align, os.path.join(valid_dir, 'step-{}-align-{}.png'.format(int(checkpoint.step), batch)))

            log('Eval loss & ler for global step {}: {:.3f}, {:.3f}'.format(int(checkpoint.step), valid_loss.result(), valid_ler.result()))
